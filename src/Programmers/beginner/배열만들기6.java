...
public class 배열만들기6 {
    ...
    /**
     * 0과 1로만 이루어진 정수 배열 arr가 주어집니다.
     * arr를 이용해 새로운 배열 answer을 만드려고 합니다.
     * i의 초기값을 0으로 설정하고 i가 arr의 길이보다 작으면 다음을 반복합니다.
     * 만약 answer이 빈 배열이라면 arr[i]를 answer에 추가하고 i에 1을 더합니다.
     *
     * answer에 원소가 있고, answer의 마지막 원소가 arr[i]와 같으면 stk의 마지막 원소를 answer에서 제거하고 i에 1을 더합니다.
     * answer에 원소가 있는데 answer의 마지막 원소가 arr[i]와 다르면 stk의 맨 마지막에 arr[i]를 추가하고 i에 1을 더합니다.
     * 위 작업을 마친 후 만들어진 answer을 return 하는 solution 함수를 완성해 주세요.
     * */
    public int[] solution(int[] arr) {
        Stack<Integer> answer = new Stack<>();
        for (int idx = 0; idx < arr.length; idx++) {
            if (!answer.isEmpty() && arr[idx] == answer.peek()) {
                answer.pop();
            } else {
                answer.add(arr[idx]);
            }
