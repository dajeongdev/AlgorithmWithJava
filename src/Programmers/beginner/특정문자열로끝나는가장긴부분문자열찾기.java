...
public class 특정문자열로끝나는가장긴부분문자열찾기 {
    ...
    public String solution(String myString, String pat) {
        for (int i = myString.length(); i > 0; i--) {
            if (myString.endsWith(pat)) break;
    ...
    public static void main(String[] args) {
        특정문자열로끝나는가장긴부분문자열찾기 T = new 특정문자열로끝나는가장긴부분문자열찾기();

        System.out.println(T.solution("AbCdEFG", "dE"));
        System.out.println(T.solution("AAAAaaaa", "a"));
        System.out.println(T.otherSolution("AbCdEFG", "dE"));
        System.out.println(T.otherSolution("AAAAaaaa", "a"));
    }
}
