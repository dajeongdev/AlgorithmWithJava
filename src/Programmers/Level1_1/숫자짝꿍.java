...
public class 숫자짝꿍 {
    ...
    public String solution(String X, String Y) {
        ...
        StringBuffer bufferY = new StringBuffer(Y);
        for (int i = 0; i < X.length(); i++) {
            for (int j = 0; j < bufferY.length(); j++) {
                if (X.charAt(i) == bufferY.charAt(j)) {
                    answer += X.charAt(i);
                    bufferY.deleteCharAt(j);
                }
            }
        }
        ...
        if (answer.length() == 0) {
            ...
            if (num > 10) {
                /** 큰 숫자로 정렬..! */
                int[] nums = Arrays.stream(answer.split("")).mapToInt(Integer::parseInt).toArray();
                Integer[] temp = Arrays.stream(nums).boxed().toArray(Integer[]::new);
                Arrays.sort(temp, Collections.reverseOrder());
                answer = Arrays.toString(temp).replaceAll("[^0-9]","");
            } else {
                answer = String.valueOf(num);
            }
        }
        /**
         * 몇 개는 틀리다고 하고 몇 개는 시간 초과 난다.. 내일 다시 한 번 보자
         */
        return answer;
    }
