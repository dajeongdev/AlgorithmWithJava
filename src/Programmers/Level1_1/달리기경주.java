...
public class 달리기경주 {
     ...
     *
     *  hashmap의 callings 원소에 해당하는 value와 그 앞에 인덱스를 찾아서 players의 원소를 변경해준다. (players swap!)
     * 이 때 hashmap에도 추월하는 players의 value를 -1해주고, 추월당한 players의 value는 +1 해준다.
     */
    public String[] solution(String[] players, String[] callings) {
        ...
        for (int i = 0; i < callings.length; i++) {
            int currentRank = playersMap.get(callings[i]);
            String player = rankMap.get(currentRank);
            String aheadPlayer = rankMap.get(currentRank - 1);

            playersMap.put(player, currentRank -1);
            playersMap.put(aheadPlayer, currentRank);

            rankMap.put(currentRank -1, player);
            rankMap.put(currentRank, aheadPlayer);
        }
        return rankMap.values().toArray(new String[players.length]);
    }

    public String[] otherSolution(String[] players, String[] callings) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(T.solution(players, callings)));
        System.out.println(Arrays.toString(T.otherSolution(players, callings)));
    }
}
