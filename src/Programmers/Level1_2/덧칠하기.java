...
public class 덧칠하기 {
     ...
     *
     * 넓은 벽 전체에 페인트를 새로 칠하는 대신, 구역을 나누어 일부만 페인트를 새로 칠함으로써 예산을 아끼려 합니다.
    ...
    // 배열 미사용 풀이
    public int otherSolution(int n, int m, int[] section) {
        int max = 0, count = 0;
        for (int s : section) {
            if (max <= s) {
                max = s + m;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(T.solution(8, 4, section1));
        System.out.println(T.solution(5, 4, section2));
        System.out.println(T.solution(4, 1, section3));
        System.out.println(T.otherSolution(8, 4, section1));
        System.out.println(T.otherSolution(5, 4, section2));
        System.out.println(T.otherSolution(4, 1, section3));
    }
}
