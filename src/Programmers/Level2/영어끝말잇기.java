...
public class 영어끝말잇기 {
    ...
    public int[] solution(int n, String[] words) {
        int[] answer = {0, 0}; // [탈락하는 사람 번호, 탈락하는 차례]
        List<String> list = new ArrayList<>();
        list.add(words[0]);

        for (int i = 1; i < words.length; i++) {
            if (words[i].length() >= 2) {
                /** 앞 단어의 마지막에 이어지지 않으면 탈락**/
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(T.solution(n1, words1)));
        System.out.println(Arrays.toString(T.solution(n2, words2)));
        System.out.println(Arrays.toString(T.solution(n3, words3)));
    }
}
