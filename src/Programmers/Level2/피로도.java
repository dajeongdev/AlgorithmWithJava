...
public class 피로도 {
    ...
    private void dfs(int[][] dungeons, int k, int i, boolean[] visited, int depth, int r, int count) {
        for (int j = 0; j < dungeons.length; j++) {
            int min = dungeons[j][0], consumption = dungeons[j][1];
            if (!visited[j] && k >= min) {
                visited[j] = true;
                dfs(dungeons, k - consumption, j, visited, depth + 1, dungeons.length, count + 1);
                visited[j] = false;
            }
        }
        answer.add(count);
    ...
    public static void main(String[] args) {
        ...
        System.out.println(T.solution(80, dungeons));
    }
}
