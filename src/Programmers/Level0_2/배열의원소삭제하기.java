...
public class 배열의원소삭제하기 {
    ...
    public int[] solution(int[] arr, int[] delete_list) {
        ...
        for (int i = 0; i < delete_list.length; i++)
            if (answer.contains(delete_list[i]))
                answer.remove(Integer.valueOf(delete_list[i])); // remove() 안에 숫자만 넣으면 인덱스로 인식함
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(T.solution(arr1, deleteList1)));
        System.out.println(Arrays.toString(T.solution(arr2, deleteList2)));
        System.out.println(Arrays.toString(T.otherSolution(arr1, deleteList1)));
        System.out.println(Arrays.toString(T.otherSolution(arr2, deleteList2)));
    }
}
