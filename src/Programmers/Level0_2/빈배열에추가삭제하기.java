...
public class 빈배열에추가삭제하기 {
    ...
    public int[] otherSolution1(int[] arr, boolean[] flag) {
        String answer = "";
        for (int i = 0; i < arr.length; i++) {
            if (flag[i]) {
    ...
    public int[] otherSolution2(int[] arr, boolean[] flag) {
    ...
    public static void main(String[] args) {
        빈배열에추가삭제하기 T = new 빈배열에추가삭제하기();

        int[] arr = {3, 2, 4, 1, 3};
        boolean[] flag = {true, false, true, false, false};
        System.out.println(Arrays.toString(T.solution(arr, flag)));
        System.out.println(Arrays.toString(T.otherSolution1(arr, flag)));
        System.out.println(Arrays.toString(T.otherSolution2(arr, flag)));
    }
}
