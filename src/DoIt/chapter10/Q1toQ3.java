...
public class Q1toQ3<K,V> {
    ...
    // 비교자로 키 값을 비교
    public Q1toQ3(Comparator<? super K> c) {
        this();
        comparator = c;
    }
    ...
    // node를 뿌리로 하는 서브 트리의 노드를 키 값의 내림차순으로 출력함
    private void printSubtreeReverse(Node node) {
        if (node != null) {
            printSubtreeReverse(node.right); // 오른쪽 서브 트리를 키 값의 내림차순으로 출력
            System.out.println(node.key.toString() + " " + node.data.toString()); // node를 나타냄
            printSubtreeReverse(node.left); // 왼쪽 서브 트리를 키 값의 내림차순으로 출력
        }
    }
