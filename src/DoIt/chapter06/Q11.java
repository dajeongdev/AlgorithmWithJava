...
// QuickSort2를 수정하여 푸시, 팝, 분할 과정을 출력하는 프로그램을 작성하세요.
public class Q11 {
    ...
    static void quickSort(int[] a, int left, int right) {
        ...
        lStack.push(left);
        rStack.push(right);
        System.out.printf("a[%d]~a[%d]를 나누는 문제를 스택에 푸시합니다. \n", left, right);
        System.out.printf("lStack : "); lStack.dump();
        System.out.printf("rStack : "); rStack.dump();

        while (lStack.isEmpty() != true) {
            int pl = left = lStack.pop();  // 왼쪽 커서
            int pr = right = rStack.pop(); // 오른쪽 커서
            int x = a[(left + right) / 2]; // 피벗
            System.out.printf("스택에서 나누는 문제를 꺼냈습니다. a[%d]~a[%d]를 나눕니다.\n", left, right);
